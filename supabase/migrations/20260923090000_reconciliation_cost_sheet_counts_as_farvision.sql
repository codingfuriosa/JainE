-- Farvision omits a unit from the Outstanding Summary when it has nothing outstanding.
--
-- Mr Aniruddha Mukherjee's unit 5D/BLOCK A1 is billed 6,247,674 and received 6,247,674 on its Sales
-- Details cost sheet - fully settled, so no Outstanding row exists for him. The portal read that
-- absence as "no Farvision figures" and withheld his statement behind the reconciling notice. He has
-- one active booking and owes nothing; there was nothing to withhold.
--
-- All 39 units in that position have a cost sheet, and the cost sheet carries Farvision's own
-- bill_amount / received_amount / balance_amount per component - the same export family, the same
-- day, equally authoritative. So absence of a snapshot is not absence of figures.
--
-- New status `cost_sheet_only` says exactly that: no snapshot to compare against, but Farvision's
-- figures are available and should be shown. `unchecked` now means what it should - no Farvision
-- figures of any kind - and currently applies to nothing.
--
-- Also exposes fv_cost_sheet_balance / fv_cost_sheet_received so the fallback is visible when
-- someone opens a row to see where a number came from. Appended at the end of the select list
-- because CREATE OR REPLACE VIEW cannot insert a column in the middle of an existing one.
...
select
  ...
  case
    ...
    when coalesce(b.amt,0) = 0 and coalesce(rc.allocated,0) = 0
         and coalesce(rc.on_account,0) = 0 and coalesce(rv.amt,0) = 0 then 'no_activity'
    else 'unchecked'
  end as status,
...
comment on view cust.reconciliation is
  'Per-unit agreement between the portal and Farvision. status: matched / mismatched (an Outstanding '
  'snapshot exists and was compared) / cost_sheet_only (no snapshot - Farvision omits units with '
  'nothing outstanding - but its Sales Details cost sheet carries billed/received/balance, which is '
  'equally authoritative) / no_activity / unchecked (no Farvision figures of any kind).';
