-- FORWARD-ONLY PROCESSING BOUNDARY.
-- From 2026-09-24 00:00 IST, a CRM snapshot queues only calls from that snapshot's own
-- decision day. Historical calls included in the CRM response remain stored but are not newly
-- transcribed or assessed. Existing queue, transcript, QA, mismatch-history and summary rows are
...
declare
  v_snap_date date;
  v_effective_date constant date := date '2026-09-24';
  v_n integer;
  v_start bigint;
  v_ins integer := 0;
  v_sales integer := 0;
...
language sql stable security definer set search_path = acc, public as $$
  ...
  in_scope_leads as (
    ...
    where q.snapshot_date = l.d
      and q.snapshot_date >= date '2026-09-24'
  )
