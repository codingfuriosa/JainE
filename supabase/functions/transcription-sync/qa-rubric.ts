...
// from the sales team's own WhatsApp assistant configurations - the funnel (greet -> buying intent
// -> configuration -> budget -> match -> ask for the visit -> confirm time -> share the address),
// the information the team is instructed never to give out over a message, and the tone rules. The
// same funnel appears identically across every project's configuration, which is what makes it a
// company standard rather than one project's habit.
//
// WHY THERE ARE NO FIGURES IN THIS FILE, AND WHY THERE MUST NEVER BE ANY.
// CATALOGUE in index.ts had to stay deliberately thin because an earlier build read the list's own
// prices back as if it had heard them on a call about neither project. Every rule below is about
// what the AGENT DID - asked, answered, confirmed, disclosed - and none of it is a fact about a
// project. There is no price here, no square footage, no location and no possession date, so there
// is nothing in this text for a transcript to absorb. That is the property that makes it safe to
// send on every call; keep it that way, and put project facts in CATALOGUE where the guards can see
// them.
//
// SCORING. "Pass", "Fail" or "Partial", with "evidence" quoting the conversation. A step that never
// arose because the customer ended the call in the first few seconds is a note, not a failure -
// the agent cannot be marked down for a question they were never given room to ask.
...
2. Etiquette - how the agent conducted themselves.
   ...
   Fail    - talked over the customer, was curt or argumentative, or hung up on a refusal without a
             polite close.
   Language mirroring belongs here: if the customer speaks Bengali or Hindi, the agent is expected
   to answer in that language. An agent who continues in English while the customer struggles is at
   best Partial.
...
3. Query Handling - were the customer's actual questions answered, accurately?
   ...
   Asking for a configuration the project does not offer is handled by saying so plainly and
   offering what does exist - not by pretending it is available.
...
7. Hyper-personalization - did the agent use what the customer told them?
   ...
   home-loan option and keep the lead, not to close the conversation. A budget above the range is
   not a problem at all - the customer stays qualified and is still asked for the visit.`;
