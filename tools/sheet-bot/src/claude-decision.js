 ...
 *  API -- not Claude Code, not a browser tool -- used only for this one judgment call;
...
export async function pickPrimaryApplicantAadhaar(candidates, applicantName, applicantPan) {
  ...
  if (!res.ok) {
    logger.warn('claude-decision: API call failed', { status: res.status });
    return null;
  }
  ...
  try {
    ...
    logger.warn('claude-decision: response was not valid JSON', { text });
    return null;
  }

  const candidateNumbers = candidates.map((c) => c.number);
  if (!candidateNumbers.includes(parsed.aadhaar_number)) {
    logger.warn('claude-decision: answer was not one of the candidates, discarding', {
      answer: parsed.aadhaar_number,
    });
    return null;
  }
