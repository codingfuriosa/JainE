...
(function(){
  ...
  async function wfRenderList(){
    ...
    // owner_email alone is only ever the FIRST of a multi-owner step's people — owner_emails
    // carries the rest. Reading only the singular column here dropped everyone but the first
    // person from this list page's avatar row for any shared step.
    try{ const {data}=await ACC().from('flow_steps').select('flow_id,owner_email,owner_emails'); (data||[]).forEach(function(s){
      ...
      owners.forEach(function(e){
        (ownersByFlow[s.flow_id]=ownersByFlow[s.flow_id]||[]);
        if(!ownersByFlow[s.flow_id].some(function(x){return eq(x,e);})) ownersByFlow[s.flow_id].push(e);
      });
    }); }catch(e){}
          ...
          +'<div class="wf-fgrid">'
            ...
            +'<div class="wf-fld">'
              ...
              +'<div id="wfTrigOwner" class="wf-owner-pick">'
                +wfPersonPickerHtml(flow.trigger_owner==='__ALL__'?[WF_ALL_PEOPLE_KEY]:(flow.trigger_owner?flow.trigger_owner.split(',').map(function(x){return x.trim();}).filter(Boolean):''), true, false, undefined, true)
              +'</div>'
            +'</div>'
  ...
  async function wfDetailPage(v, id, selCaseId){
    ...
    // '__ALL__'/comma-list trigger_owner needs splitting — pushing the raw string produced garbled
    // "avatars" for anything but a single real email. Same for steps: owner_email is only ever the
    // FIRST of a multi-owner step's people (owner_emails carries the rest) — read both, or a
    // multi-owner step only ever showed one profile picture here.
    const members=[];
    if(flow.trigger_owner && flow.trigger_owner!=='__ALL__'){
      flow.trigger_owner.split(',').map(function(x){return x.trim();}).filter(Boolean).forEach(function(e){
        if(!members.some(function(m){return eq(m,e);})) members.push(e);
      });
    }
    steps.forEach(function(s){
      const owners=(Array.isArray(s.owner_emails)&&s.owner_emails.length)?s.owner_emails:(s.owner_email?[s.owner_email]:[]);
      owners.forEach(function(e){ if(e&&!members.some(function(m){return eq(m,e);}))members.push(e); });
    });
  ...
  window.wfEventOpen=async function(flowId, caseId){
    ...
    // A workflow can have several steps all set to "decided per instance" (e.g. Invoice
    // Processing's Dept Check and Sign Cheque Sent Back). These used to get one picker EACH,
    // requiring the same person to be chosen redundantly per step — now it's one picker, required,
    // and whoever's chosen there is applied to every such step.
    const membersHtml=openSteps.length
      ? '<label class="wf-lbl">Who does '+(openSteps.length>1?'these steps':'this step')+'? '+tip((openSteps.length>1?'These steps (':'This step (')+openSteps.map(function(st){return st.title||('Step '+st.seq);}).join(', ')+(openSteps.length>1?') have':') has')+' no fixed owner — decided when the '+N.lc+' is started. Name one or more people; they all receive it, and the first to accept it keeps it.')+'</label>'
        +'<div id="wfEvtMembers" class="wf-evt-row wf-mem-row" data-seqs="'+openSteps.map(function(st){return st.seq;}).join(',')+'">'
          +'<div class="ac-in wf-evt-labelro wf-mem-lbl" style="background:#f8fafc;color:var(--ink);display:flex;align-items:center;gap:7px;overflow:hidden">'
            ...
            +'<span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc2(openSteps.map(function(st){return st.title||('Step '+st.seq);}).join(' / '))+'</span></div>'
  ...
  async function wfEventSaveInner(flowId, caseId){
    ...
    try{
      if(caseId){
        ...
        // One picker covers every "decided per instance" step at once — same people applied to
        // all of them, and required (can't leave a step silently unassigned).
        const members={};
        const memBox=$('wfEvtMembers');
        if(memBox){
          const seqs=(memBox.getAttribute('data-seqs')||'').split(',').map(function(x){return x.trim();}).filter(Boolean);
          const v=((memBox.querySelector('.wf-s-person')||{}).value||'').split(',').map(function(x){return x.trim();}).filter(Boolean);
          if(!v.length){ toast('Please choose who does '+(seqs.length>1?'these steps':'this step'),'warn'); return; }
          seqs.forEach(function(seq){ members[seq]=v; });
        }
